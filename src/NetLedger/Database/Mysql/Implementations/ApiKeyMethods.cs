namespace NetLedger.Database.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of API key methods.
    /// </summary>
    internal class ApiKeyMethods : IApiKeyMethods
    {
        ...
        internal ApiKeyMethods(MysqlDatabaseDriver driver)
        {
        ...
        public async Task<EnumerationResult<ApiKey>> EnumerateAsync(EnumerationQuery query, CancellationToken token = default)
        {
            ...
            mainQuery.Append(" " + filter.GetLimitOffsetClause(DatabaseTypeEnum.Mysql));
            mainQuery.Append(";");
        ...
        private ApiKey DataRowToApiKey(DataRow row)
        {
            ...
            apiKey.Active = Convert.ToBoolean(row["active"]);
            apiKey.IsAdmin = Convert.ToBoolean(row["isadmin"]);
