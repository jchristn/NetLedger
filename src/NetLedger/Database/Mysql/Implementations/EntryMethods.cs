namespace NetLedger.Database.Mysql.Implementations
{
    using System;
    ...
    /// <summary>
    /// MySQL implementation of entry methods.
    /// </summary>
    internal class EntryMethods : IEntryMethods
    {
        ...
        internal EntryMethods(MysqlDatabaseDriver driver)
        {
        ...
        {
            string query = "SELECT * FROM `entries` WHERE `accountguid` = '" + Sanitize(accountGuid.ToString()) + "' AND `type` = 'Balance' ORDER BY `createdutc` DESC LIMIT 1;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        {
            string query = "SELECT * FROM `entries` WHERE `accountguid` = '" + Sanitize(accountGuid.ToString()) + "' AND `type` = 'Balance' AND `createdutc` <= '" + asOfUtc.ToString(SetupQueries.TimestampFormat) + "' ORDER BY `createdutc` DESC LIMIT 1;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<List<Entry>> ReadWithFilterAsync(Guid accountGuid, FilterBuilder filter, CancellationToken token = default)
        {
            ...
            query.Append(" " + filter.GetOrderByClause(DatabaseTypeEnum.Mysql));
            query.Append(" " + filter.GetLimitOffsetClause(DatabaseTypeEnum.Mysql));
            query.Append(";");
        ...
        public async Task<EnumerationResult<Entry>> EnumerateAsync(Guid accountGuid, EnumerationQuery query, CancellationToken token = default)
        {
            ...
            {
                mainQuery.Append(" " + filter.GetOrderByClause(DatabaseTypeEnum.Mysql));
            }
        ...
        {
            string query = "SELECT COALESCE(SUM(`amount`), 0) FROM `entries` WHERE `accountguid` = '" + Sanitize(accountGuid.ToString()) + "' AND `type` = 'Credit' AND `iscommitted` = 0;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        {
            string query = "SELECT COALESCE(SUM(`amount`), 0) FROM `entries` WHERE `accountguid` = '" + Sanitize(accountGuid.ToString()) + "' AND `type` = 'Debit' AND `iscommitted` = 0;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
