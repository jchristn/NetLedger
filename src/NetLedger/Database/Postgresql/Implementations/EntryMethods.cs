namespace NetLedger.Database.Postgresql.Implementations
{
    ...
    internal class EntryMethods : IEntryMethods
    {
        ...
        {
            string query = "SELECT COALESCE(SUM(amount), 0) FROM entries WHERE accountguid = '" + Sanitize(accountGuid.ToString()) + "' AND type = 'Credit' AND iscommitted = FALSE;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        {
            string query = "SELECT COALESCE(SUM(amount), 0) FROM entries WHERE accountguid = '" + Sanitize(accountGuid.ToString()) + "' AND type = 'Debit' AND iscommitted = FALSE;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
