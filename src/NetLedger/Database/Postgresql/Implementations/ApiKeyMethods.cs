namespace NetLedger.Database.Postgresql.Implementations
{
    ...
    /// <summary>
    /// PostgreSQL implementation of API key methods.
    /// </summary>
    internal class ApiKeyMethods : IApiKeyMethods
    {
        ...
        internal ApiKeyMethods(PostgresqlDatabaseDriver driver)
        {
        ...
        public async Task<EnumerationResult<ApiKey>> EnumerateAsync(EnumerationQuery query, CancellationToken token = default)
        {
            ...
            mainQuery.Append(" " + filter.GetLimitOffsetClause(DatabaseTypeEnum.Postgresql));
            mainQuery.Append(";");
        ...
        public async Task<bool> ExistsActiveKeyAsync(string key, CancellationToken token = default)
        {
            ...
            string query = "SELECT COUNT(*) FROM apikeys WHERE apikey = '" + Sanitize(key) + "' AND active = TRUE;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<ApiKey?> AuthenticateAsync(string key, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM apikeys WHERE apikey = '" + Sanitize(key) + "' AND active = TRUE LIMIT 1;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
