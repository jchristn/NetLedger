namespace NetLedger.Database.Postgresql.Implementations
{
    ...
    /// <summary>
    /// PostgreSQL implementation of account methods.
    /// </summary>
    internal class AccountMethods : IAccountMethods
    {
        ...
        internal AccountMethods(PostgresqlDatabaseDriver driver)
        {
        ...
        public async Task<List<Account>> SearchByNameAsync(string searchTerm, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM accounts WHERE name ILIKE '%" + Sanitize(searchTerm) + "%' ORDER BY createdutc DESC;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
